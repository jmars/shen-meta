...
(define zinc-t { klambda --> zinc-code }
  ...
  [F | Args]   <- (if (primitive? F)
                    (append (fold-append [] (intersperse [push] (map-zinc-c (reverse (tl Args)))))
                            (append [push] (append (zinc-c (hd Args)) [prim F])))
                    (fail)) where (symbol? F)
  [F | Args]   -> (append (fold-append [] (intersperse [push] (map-zinc-c (reverse Args))))
                          (append [push] (append (zinc-c F) [appterm])))
...
(define zinc-c { klambda --> zinc-code }
  ...
  [F | Args]   -> (append [pushmark]
                          (append (fold-append [] (intersperse [push] (map-zinc-c (reverse Args))))
                                  (append [push] (append (zinc-c F) [apply]))))
  X            -> [boolean X] where (boolean? X)
  X            -> [number X] where (number? X)
  X            -> [string X] where (string? X)
  []           -> []
