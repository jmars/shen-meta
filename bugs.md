...
- **Safe wrappers now cover:** all the arithmetic (`+ - * /`, incl. division-by-zero), list (`hd`/`tl`/`fst`/`snd`/`cons`/`emptylist`), string (`n->string`/`string->n`/`tlstr`/`str`), symbol (`intern`/`value`/`set`), vector (`absvector` incl. negative, `<-address`/`address->`), I/O (`open` incl. genuine open-failure, `close`, `read-byte`, `write-byte`), `get-time`, `pos`, `cn`, comparisons, `trap-error`, `simple-error`, `error-to-string`, and the type predicates. `hdstr` and `read-file-as-string` are **not** wrapped (they lack metacircular-interp `[prim ...]` rules, so wrapping them breaks self-hosting) — their errors stay C-level debug-only defense.
