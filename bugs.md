...
- **Type-error primitives only throw inside an active trap-error body** (`vm_catch_chain && in_trap_error`). Outside any trap they keep the legacy `fprintf(stderr, ...); return -1` behavior, so `trap-error` cannot catch a type error raised in top-level (non-trapped) code. This preserves built-in test behavior. As of `PRIM_TYPE_ERROR` (commit pending), essentially all primitive type-check sites now route through `vm_throw` when a catch frame is active: `fst`/`snd`/`hd`/`tl` non-cons, `+ - * /` non-numbers, division-by-zero, `emptylist`, `n->string`/`string->n`/`tlstr`/`hdstr`/`intern`, `absvector`, `address->`, `<-address`, `pos`, `value`, `read-byte`/`write-byte`, `get-time`, `set`, `open` bad-types, `close` non-stream, `read-file-as-string` non-string, `trap-error` handler not fn, `env_pop`, and `apply`/`appterm` non-callable. The remaining `fprintf+return -1` sites are **resource/internal** errors, deliberately not routed: file-open failures, `open` direction value, string-stream index invariants, and eval-kl "closure not in bundle". To make `trap-error` catch those too, they would need explicit `vm_throw` routing (they are not type-check errors, so this is low priority).
