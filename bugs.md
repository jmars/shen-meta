...
- **The C type-checks are NOT compiled out in release.** Compiling out the pure type-tag checks (`PRIM_TYPE_CHECK` debug-only, `PRIM_VALUE_GUARD` always-on) makes release self-hosting segfault during `shen.initialise`: even though a `ZINCVM_DEBUG` run shows no type-tag fires, the release build crashes at the `value` primitive with a garbage symbol (heap/state corruption from an earlier unchecked operation that the debug checks mask). The static path is not yet provably type-safe in practice, so the C checks must stay.
