...
- **The C type-checks are NOT compiled out in release.** The `.kl` Shen OS kernel (`shen.initialise` etc.) contains type-unsafe arithmetic calls (`+ - * /` on non-numbers) that reach raw `[prim X]` and rely on the C check returning `-1` (recovered). Removing them causes a segfault during `shen.initialise` (verified). The static path would have to route through the safe wrappers before the C checks could be dropped.
